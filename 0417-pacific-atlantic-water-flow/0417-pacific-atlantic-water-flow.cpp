class Solution {
public:
    void dfs(int i, int j, vector<vector<int>>& height,
        ...
        if (i < n - 1 && height[i + 1][j] >= height[i][j] &&
            ocean[i + 1][j] == false)
            dfs(i + 1, j, height, ocean);
        if (j < m - 1 && height[i][j + 1] >= height[i][j] &&
            ocean[i][j + 1] == false)
            dfs(i, j + 1, height, ocean);
        return;
    }
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        // If water can flow from a cell in both atlantic and pacific add that
        // to our answer.
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
